...
export default function ArbitrageDemoPage() {
  ...
  const run = useCallback(async () => {
    ...
    setResult(null);
    try {
      let poolsToUse: Pool[] = pools.length ? pools : (await fetchPharosPools()) as Pool[];
    ...
    <div className="bg-pharos-dark text-slate-200">
      <div className="container mx-auto px-4 py-8">
        <h1 className="mb-2 text-3xl font-bold text-white">
          High-frequency consolidation: optimal pathfinding
        </h1>
        <p className="mb-8 text-slate-400">
          Efficient processing of swap paths across many liquidity pools (DEX order book / Reorg DEX).
          Full algorithm visualization: tokens = nodes, pools = edges; optimal path highlighted.
          On complex graphs (5+ currencies), classical greedy (2-hop) reaches a local optimum;
          quantum-inspired full path search finds the global optimum with higher output.
        </p>
